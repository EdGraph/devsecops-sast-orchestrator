...
- Implements real model routing based on request criticality: **Gemini 3.1 Flash-Lite** for the Low/Medium specificity levels, and **Gemini 3.1 Pro** for the High-criticality level, balancing cost and output quality (*FinOps*).
...
Manages evidence and report file traffic between the local workspace and the cloud.
...
- **Cloud APIs:** Google GenAI SDK (Gemini 3.1 Pro + Gemini 3.1 Flash-Lite, routed by criticality), Google Drive API v3, SonarCloud REST API
...
### 🔐 Security Note
This project relies on OAuth2 credentials (`credentials.json`), a SonarCloud token, and a Gemini API key. **None of these should ever be committed to the repository.** Keep them out of version control (via `.gitignore`) and load sensitive values from environment variables rather than hardcoding them in the scripts.
...
# 3. Execute the AI refactoring pipeline (Flash-Lite/Pro routed by criticality)
...
### 📊 Results
Running the pipeline produces two CSV reports for comparative analysis: `resultados_baseline.csv` (original code metrics) and `resultados_pesquisa_sast.csv` (metrics per refactoring level and attempt). These files can be used to compute mitigation effectiveness across specificity levels.
...
- Implementa roteamento real de modelos por criticidade: **Gemini 3.1 Flash-Lite** para os níveis Baixa/Média especificidade, e **Gemini 3.1 Pro** para o nível de Alta criticidade, equilibrando custo e qualidade de saída (*FinOps*).
...
Gerencia o tráfego de arquivos de evidências e relatórios entre o ambiente local e a nuvem.
...
- **APIs Cloud:** Google GenAI SDK (Gemini 3.1 Pro + Gemini 3.1 Flash-Lite, roteados por criticidade), Google Drive API v3, SonarCloud REST API
...
### 🔐 Nota de Segurança
Este projeto depende de credenciais OAuth2 (`credentials.json`), de um token do SonarCloud e de uma chave de API do Gemini. **Nenhuma dessas credenciais deve ser commitada no repositório.** Mantenha-as fora do versionamento (via `.gitignore`) e carregue valores sensíveis a partir de variáveis de ambiente, em vez de deixá-los fixos no código.
...
# 3. Executa o pipeline de refatoração com IA (roteamento Flash-Lite/Pro por criticidade)
python 03_llm_refactoring_pipeline.py
...
### 📊 Resultados
Ao rodar o pipeline, são gerados dois relatórios CSV para análise comparativa: `resultados_baseline.csv` (métricas do código original) e `resultados_pesquisa_sast.csv` (métricas por nível de refatoração e tentativa). Esses arquivos podem ser usados para calcular a eficácia de mitigação entre os níveis de especificidade.
